...
class LocalStDev(LocalAffinity):
    ...
    def forward(self, x):
        ...
        x = super(LocalStDev, self).forward(x , state='mask')
        return x.std(2, keepdim=True)


class LocalAffinityAbs(LocalAffinity):
    def forward(self, x):
        x = super(LocalAffinityAbs, self).forward(x ,state='input')
        return torch.abs(x)
...
class PAMR(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        for _ in range(self.num_iter):
            ...
            m = self.aff_m(mask , state='mask')  # [BxCxPxHxW]
